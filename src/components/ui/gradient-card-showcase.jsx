...
export default function GradientCardShowcase({ items }) {
    return (
        <div className="flex justify-center items-center flex-wrap gap-10 py-10">
            {items.map(({ name, role, desc, img, gradientFrom, gradientTo, github, linkedin }, idx) => (
                <div
                    key={idx}
                    className="group relative w-[300px] h-[380px] m-[10px] transition-all duration-500"
                >
                    ...
                    <span className="pointer-events-none absolute inset-0 z-10">
                        ...
                        <span className="absolute bottom-0 right-0 w-0 h-0 rounded-lg opacity-0 bg-[rgba(255,255,255,0.1)] backdrop-blur-[10px] shadow-[0_5px_15px_rgba(0,0,0,0.08)] transition-all duration-500 animate-blob group-hover:bottom-[-40px] group-hover:right-[40px] group-hover:w-[80px] group-hover:h-[80px] group-hover:opacity-100" style={{ animationDelay: '1s' }} />
                    </span>

                    {/* Content */}
                    <div className="relative z-20 left-0 h-full flex flex-col justify-between p-[20px_40px] bg-[rgba(255,255,255,0.05)] backdrop-blur-[10px] shadow-lg rounded-lg text-white transition-all duration-500 group-hover:left-[-15px] group-hover:p-[40px_40px] border border-white/10 group-hover:border-white/20">

                        {/* Top Section: Name & Role */}
                        <div className="relative z-30 transition-all duration-500 group-hover:z-10 group-hover:opacity-90 group-hover:scale-95"> {/* Text drops BACK */}
                        ...
                        {img && (
                            <div className="absolute bottom-0 left-0 right-0 h-[85%] z-20 pointer-events-none overflow-hidden rounded-b-lg transition-all duration-500 group-hover:z-40 group-hover:scale-105"> {/* Image pops FRONT */}
                                <img
                                    src={img}
                                    alt={name}
                                    className="w-full h-full object-contain object-bottom opacity-90 grayscale group-hover:grayscale-0 transition-all duration-500"
                                />
